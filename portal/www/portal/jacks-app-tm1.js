...
function JacksApp(target, status, buttons, sliceAms, allAms, sliceId) {
...
JacksApp.prototype.initEvents = function() {
    ...
    this.input.on("all", function(eventName) {
        console.log("JacksApp DEBUG input event " + eventName);
    });
    this.input.on("manifest", this.onEpManifest, this);

    this.output.on("all", function(eventName) {
        console.log("JacksApp DEBUG output event " + eventName);
    });
    this.output.on("manifest", ep_on_manifest);
}
...
JacksApp.prototype.initButtons = function(buttonSelector) {
    ...
    btn.click(function(){ jacks_get_manifest();});
...
JacksApp.prototype.onEpManifest = function(event) {
    ...
    jacksXml.find('node').each(function(i, v) {
        ...
        $(this).find('login').each(function(il, vl) {
            ...
            console.log(authn + ": " + hostname);
        });
    });

    jacksAMs[jacksAllAms[am_id]['name']] = [];
    jacks_poll_status(am_id);
}
...
function start_jacks_viewer(target, status, buttons, sliceAms, allAms, sliceId) {
  ...
  myJacksApp = new JacksApp();

  // FIXME: A temporary global as the code is transitioned to an
  // object-oriented model.
  theJacksApp = myJacksApp;
  ...
  // Initialize the Jacks Viewer instance.
    var jacksInstance = new window.Jacks({
        ...
        readyCallback: function (input, output) {
            myJacksApp.jacksReady(input, output);
    ...
    myJacksApp.initButtons(jacksButtons);

}

function ep_jacks_app_ready() {
}
function ep_on_manifest(event) {
    ...
    $.get("jacks-app-details.php",
          ...
          function(responseTxt, statusTxt, xhr) {
              rspec = responseTxt;
              event.callback.trigger("manifest",
                                     { am_id: am_id,
                                       slice_id: slice_id,
                                       rspec: rspec
                                     });
...
function jacks_get_manifest() {
  ...
  $.each(jacksSliceAms, function(i, am_id) {
    // Update the status bar.
    jacks_update_status('Gathering manifest from '+am_id+'...');
    theJacksApp.output.trigger("manifest", { am_id: am_id,
                                             slice_id: jacksSliceId,
                                             callback: theJacksApp.input
                                           });
    // Ajax call for the manifest.
    // $.get("jacks-app-details.php", { am_id:am_id, slice_id:jacksSliceId }, function(responseTxt,statusTxt,xhr) {
...
//
    ...
    //     theJacksApp.client2am[$(this).attr('sliver_id')] = am_id;
    //     // This is needed because some AMs do return the client_id, so
    //     // the mapping needs to have both to avoid needing special cases.
    //     theJacksApp.client2am[$(this).attr('client_id')] = am_id;
